...
L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}', { //Initialize map with mapbox streets tileset and min/max zoom level
    ...
    mymap.locate({setView: inBound , watch: true})
     ...
     // TODO make icons relative to current zoom: iconSize:[mymap.getZoom()*.85, mymap.getZoom()*.90]})
     ...
     for (i = 0; i<90; i++) {
        ...
        var marker = L.marker([lat, lon], {icon: markerIcon}).addTo(mymap).bindPopup("<h4>"+street+"</h4><hr/><p>Address: "+street+" "+city+", "+state+" "+zip+" <br/> Hours: MWF 9:00 am to 5:00 pm <br/> </p>"); //Add marker with custom icon to map
     }
